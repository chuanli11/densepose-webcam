...
def TransferTexturePure(TextureIm,im,IUV):
    U = IUV[:,:,1]
    V = IUV[:,:,2]
    #
    R_im = np.zeros(U.shape)
...
def main(args):
    ...
    # Set Texture
    Tex_Atlas = cv2.imread('DensePoseData/demo_data/texture_from_SURREAL.png')[:,:,::-1]/255.0
    ...
    while True:
        ...
        torch_end.record()
        print("infer_engine {}".format(end - start))
        torch.cuda.synchronize()
        print("torch infer_engine {}".format(torch_start.elapsed_time(torch_end) / 1000.0))
        ...
        print("vis_webcam {}".format(end - start))
        ...
        total_end = time.time()
        print("FPS {}".format(1 / (total_end - total_start)))
